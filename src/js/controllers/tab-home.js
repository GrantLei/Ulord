...
angular.module('copayApp.controllers').controller('tabHomeController',
  function($rootScope, $timeout, $scope, $state, $stateParams, $ionicModal, $ionicScrollDelegate, $window, gettextCatalog, lodash, popupService, ongoingProcess, externalLinkService, latestReleaseService, profileService, walletService, configService, $log, platformInfo, storageService, txpModalService, appConfigService, startupService, addressbookService, feedbackService, bwcError, nextStepsService, buyAndSellService, homeIntegrationsService, bitpayCardService, pushNotificationsService, timeService) {
    ...
    var updateAllWallets = function() {
      ...
      $scope.walletsDash = profileService.getWallets({coin: 'dash'});
      $scope.walletsNano = profileService.getWallets({coin: 'Nano'});
      $scope.walletsUlord = profileService.getWallets({coin: 'Ulord'});
      ...
      lodash.each($scope.walletsDash, function(wDash) {
        wallets.push(wDash);
      });

      lodash.each($scope.walletsNano, function(wNano) {
        wallets.push(wNano);
      });

      lodash.each($scope.walletsUlord, function(wUlord) {
        wallets.push(wUlord);
      });
 ...
 var result = [];
        ...
        if(notifications) {
          if (notifications.length >= 2) {
            for (var i = 0, leni = notifications.length; i < leni; i++) {
              isRepeated = false;
              for (var j = 0, lenj = result.length; j < lenj; j++) {
                if (notifications[i]["createdOn"] == result[j]["createdOn"]) {
                  isRepeated = true;
                  break;
                }
              }
