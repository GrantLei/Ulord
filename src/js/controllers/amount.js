...
angular.module('copayApp.controllers').controller('amountController', function($scope, $filter, $timeout, $ionicScrollDelegate, $ionicHistory, gettextCatalog, platformInfo, lodash, configService, rateService, $stateParams, $window, $state, $log, txFormatService, ongoingProcess, popupService, bwcError, payproService, profileService, bitcore, amazonService, nodeWebkitService) {
  ...
  $scope.$on("$ionicView.beforeEnter", function(event, data) {
    ...
    function setAvailableUnits() {
      ...
      var hasDASHWallets = profileService.getWallets({
        coin: 'dash'
      }).length;
      ...
      var hasNANOWallets = profileService.getWallets({
        coin: 'Nano'
      }).length;
      ...
      var hasUTWallets = profileService.getWallets({
        coin: 'Ulord'
      }).length;
  ...
  function processAmount() {
    ...
    if (lodash.isNumber(result)) {
      ...
      if (availableUnits[unitIndex].isFiat) {
        ...
        if (a) {
          $scope.alternativeAmount = txFormatService.formatAmount(a * satoshi, true);
        } else {
          if (result) {
        ...
        $scope.alternativeAmount = $filter('formatFiatAmount')(result * satoshi);
      }
    }
  };
  ...
  function processResult(val) {
    ...
    else return txFormatService.formatAmount(val.toFixed(unitDecimals) * satoshi, true);
  };
  ...
  function toFiat(val) {
    ...
    return parseFloat((rateService.toFiat(val * satoshi, fiatCode, availableUnits[unitIndex].id)).toFixed(fix));
  };
