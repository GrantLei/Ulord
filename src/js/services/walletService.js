...
angular.module('copayApp.services').factory('walletService', function($log, $timeout, lodash, trezor, ledger, intelTEE, storageService, configService, rateService, uxLanguage, $filter, gettextCatalog, bwcError, $ionicPopup, fingerprintService, ongoingProcess, gettext, $rootScope, txFormatService, $ionicModal, $state, bwcService, bitcore, popupService, feeService) {
  ...
  root.getStatus = function(wallet, opts, cb) {
    ...
    function cacheBalance(wallet, balance) {
      ...
      //STR
      var coinStyle;
      if(wallet.coin == 'Ulord'){
        coinStyle = 'ULD';
      }else{
        coinStyle = wallet.coin;
      }
      cache.totalBalanceStr = txFormatService.formatAmountStr(coinStyle, cache.totalBalanceSat);
      cache.lockedBalanceStr = txFormatService.formatAmountStr(coinStyle, cache.lockedBalanceSat);
      cache.availableBalanceStr = txFormatService.formatAmountStr(coinStyle, cache.availableBalanceSat);
      cache.spendableBalanceStr = txFormatService.formatAmountStr(coinStyle, cache.spendableAmount);
      cache.pendingBalanceStr = txFormatService.formatAmountStr(coinStyle, cache.pendingAmount);
...
console.log('进行签名验证');
  ...
  root.getProtocolHandler = function(wallet) {
    ...
    if (wallet.coin == 'Nano') return 'bitcoinnano';
    if (wallet.coin == 'dash') return 'bitcoindash';
    if (wallet.coin == 'Ulord') return 'bitcoinulord';
    else return 'bitcoin';
  }
